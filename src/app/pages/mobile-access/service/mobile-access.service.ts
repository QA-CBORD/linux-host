...
import {
    BaseService,
    ServiceParameters,
} from 'src/app/core/service/base-service/base.service';
...
export class MobileAccessService extends BaseService {
    ...
    private coords: MGeoCoordinates = { latitude: null, longitude: null, accuracy: null};
    ...
    getMobileLocations(incomeGeoData: MGeoCoordinates): Observable<MMobileLocationInfo[]> {
        ...
        return this.httpRequest(
            ...
            map(({ response }) => this._locations = response),
        );
    }
    ...
    activateMobileLocation(
        ...
        const postParams = this.createMobileLocationParams(
            locationId,
            geoData,
            sourceInfo
        );
     ...
     * @param sourceInfo
     */
    private createMobileLocationParams(
        locationId: string,
        geoData: any,
        sourceInfo: string
    ) {
        ...
        const latitude =
            isGeoDataNull || geoData.coords.latitude == null
                ? null
                : geoData.coords.latitude;
        const longitude =
            isGeoDataNull || geoData.coords.longitude == null
                ? null
                : geoData.coords.longitude;
        const accuracy =
            isGeoDataNull || geoData.coords.accuracy == null
                ? null
                : geoData.coords.accuracy;
        const altitude =
            isGeoDataNull || geoData.coords.altitude == null
                ? null
                : geoData.coords.altitude;
        const altitudeAccuracy =
            isGeoDataNull || geoData.coords.altitudeAccuracy == null
                ? null
                : geoData.coords.altitudeAccuracy;
        const heading =
            isGeoDataNull || geoData.coords.heading == null
                ? null
                : geoData.coords.heading;
        const speed =
            isGeoDataNull || geoData.coords.speed == null
                ? null
                : geoData.coords.speed;
