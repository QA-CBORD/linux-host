...
describe('ServerError', () => {
  ...
  const httpResponseStub = new HttpResponse({
    ...
    body: { exception: '9000|Internal Server Error', method: 'test' },
  });
  ...
  it('should return unexpected error occurred', async () => {
    ...
    const unknownExceptionFormat = new HttpResponse({
      status: 500,
      statusText: 'Internal Server Error',
      body: { exception: 'Internal Server Error', method: 'test' },
    });
  ...
  it('should NOT return the invalid session error', async () => {
    const unknownExceptionFormat = new HttpResponse({
      status: 500,
      statusText: 'Internal Server Error',
      body: { exception: '4001|Invalid session', method: 'test' },
    });
    const httpHandler: HttpHandler = {
      handle: (req: HttpRequest<any>) => {
        return of(unknownExceptionFormat);
