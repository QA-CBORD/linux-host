...
describe('VaultIdentityService', () => {
  let service: VaultIdentityService, injector, userPreferenceService, vault, loadingService, modalController;
  beforeEach(async () => {
    vault = {
      onError: jest.fn(),
  ...
  describe('Vault data migration', () => {
    ...
    it('should result in migrationNotNeeded when no data foud in old-vault', async () => {
      migrator.exportVault = () => {
        throw new Error('no data in legacy vault');
      };
      const migrationResult = await service.migrateIfLegacyVault();
    ...
    it('should allow pin retry on biometric failure when migrating data', async () => {
      ...
      const tryUnlock0Spy = jest
        .spyOn(vaultAutenticator, 'tryUnlock0')
        .mockResolvedValue({ pin: '1111', status: PinCloseStatus.LOGIN_SUCCESS });
      const migrateSuccessSpy = jest.spyOn(service, 'onVaultMigratedSuccess');
      const migrationResult = await service.migrateIfLegacyVault();
    ...
    it('should result in migration failed when user fails pin authentication', async () => {
      ...
      migrator.exportVault = () => {
        throw new Error('Some pin related error occurred');
      };
      const migrateSuccessSpy = jest.spyOn(service, 'onVaultMigratedSuccess');
      const migrationResult = await service.migrateIfLegacyVault();
  ...
  describe('Vault login, unlock and logout', () => {
    ...
    it('should retry unlock vault with pin when biometric auth fails', async () => {
      ...
      const pinModalSpy = jest
        ...
        .mockResolvedValue({ pin: session.pin, status: PinCloseStatus.LOGIN_SUCCESS });
    ...
    it('should logout user when pin retry fails', async () => {
      ...
      const pinModalSpy = jest
        .spyOn(service.pinAuthenticator, 'tryUnlock0')
        .mockResolvedValue({ pin: null, status: PinCloseStatus.MAX_FAILURE });
  ...
  describe('misc', () => {
    ...
    it('should result in biometricNotAvailable when user has denied permissions to use it', async () => {
      ...
      const deviceMock = jest.spyOn(Device, 'isBiometricsEnabled').mockResolvedValue(true);
      const getBiometricPermissionDeniedSpy = jest
        .spyOn(userPreferenceService, 'getBiometricPermissionDenied')
      ...
      const denyPermissionStub = jest
        .spyOn(userPreferenceService, 'setBiometricPermissionDenied')
      ...
      const isBiometricDenied2 = await service.isBiometricPermissionDenied({
        code: VaultErrorCodes.SecurityNotAvailable,
      });
    ...
    it('should get values correctly', async () => {
      jest.spyOn(service, 'getValues').mockImplementation(() => Promise.resolve({ key1: 'value1', key2: 'value2' }));
      const values = await service.getValues();
      expect(values).toEqual({ key1: 'value1', key2: 'value2' });
    });

    it('should set values correctly', async () => {
      jest.spyOn(service, 'setValues').mockImplementation(() => Promise.resolve());
      await service.setValues({ key1: 'value1', key2: 'value2' });
      expect(service.setValues).toHaveBeenCalledWith({ key1: 'value1', key2: 'value2' });
    });
  });
});
