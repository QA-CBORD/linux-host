...
import { MobileCredential } from './model/mobile-credential';
...
export class PartnerPaymentApiFacadeService {
  ...
  androidActivePasses(debug=false): Observable<MobileCredential>{
    ...
    return from(Device.getInfo()).pipe(switchMap(({ model, osVersion, manufacturer }) => {
      ...
      return forkJoin(omniIDJwtToken$, authorizationBlob$).pipe(switchMap(([jwtToken, authBlob]) => {
               return this.partnerPaymentApi.androidActivePasses(jwtToken, authBlob)
                          .pipe(map(data => {
                          return new MobileCredential(data);
                }))
          }));
   }));
}
  ...
  androidCredential(debug=false): Observable<any>{
    /**
    ...
    if(debug){
      return this.partnerPaymentApi.mockActiveAndroidCredentials();
    }
