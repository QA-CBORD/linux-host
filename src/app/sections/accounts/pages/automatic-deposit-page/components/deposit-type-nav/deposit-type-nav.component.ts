...
export class DepositTypeNavComponent implements OnInit{
  ...
  private setAvailableTypes() {
    this.availableTypes = zip(this.settingsFacadeService.getSetting(Settings.Setting.AUTO_DEPOSIT_ENABLED), this.settingsFacadeService.getSetting(Settings.Setting.LOW_BALANCE_AUTO_DEPOSIT_ENABLED)).pipe(
      map(settings => {
